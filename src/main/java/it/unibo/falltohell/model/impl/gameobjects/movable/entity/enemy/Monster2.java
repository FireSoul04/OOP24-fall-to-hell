...
public class Monster2 extends BaseEnemy {
    ...
    public Monster2(final Level level, final Vector2 initialCord, final Character character) {
        ...
        this.stats.getTm().addTimer(this.stats.getNo_aggroName(), new CustomTimerImpl(this.stats.getNoAggro(), () -> {
            if (this.isFull()) {
        ...
        this.stats.getTm().addTimer(attack, new CustomTimerImpl(4000, () -> {
            this.attack();
        ...
        while (other_X > 0) {
            if (chara.distance(super.getPosition()) > 20) {
                if (this.stats.getInitialPos()
                        .distance(new Vector2(super.getPosition().x() + (other_X * this.direction), y)) <= this.stats
                                .getDistance()) {
                    if (this.collided.isPresent() && super.getPosition().add(new Vector2(other_X * this.direction, 0))
                    ...
                    other_X -= Math.abs((this.stats.getInitialPos().x() + this.stats.getDistance() * this.direction)
                            - super.getPosition().x());
                    super.setPosition(
                ...
                if ((chara.x() <= this.stats.getDistance() + this.stats.getInitialPos().x())
                    ...
                    if (chara.distance(super.getPosition()) > super.getPosition()
                            .distance(new Vector2(super.getPosition().x() + other_X * this.direction, y))) {
                        if (chara.x() - super.getPosition().x() > 0 && !(this.collided.isPresent()
                                && this.collided.get().x() < super.getPosition().add(new Vector2(other_X, 0)).x())) {
                    ...
                    if (chara.x() - super.getPosition().x() > 0) {
                        if (this.stats.getInitialPos()
                                .distance(new Vector2(super.getPosition().x() + other_X, y)) <= this.stats.getDistance()
                                && !(this.collided.isPresent() && super.getPosition().add(new Vector2(other_X, 0))
                                        .x() > this.collided.get().x())) {
                            super.setPosition(super.getPosition().add(new Vector2(other_X, 0)));
                            other_X = 0;
