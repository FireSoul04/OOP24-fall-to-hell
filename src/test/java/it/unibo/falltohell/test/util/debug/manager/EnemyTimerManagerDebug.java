...
/**
 * A debug implementation of {@link EnemyTimeManagerImpl} for managing enemy
 * timers.
 * <p>
 * This class tracks attack and "no aggro" timers for enemies during debugging.
 * It also provides utilities to inspect and manage the timers associated with
 * each enemy.
 * </p>
 *
 * @author Sara Visani
 */
public class EnemyTimerManagerDebug extends EnemyTimeManagerImpl {
     ...
     */
    public String getNextNoAggroName(final Enemy enemy) {
     ...
     */
    public String getTimerName(final Enemy enemy, final TimerPrefix prefix) {
     ...
     */
    public String findTimer(final List<String> timers, final TimerPrefix prefix) {
        if (timers == null) {
            return null;
        }
        for (final String timer : timers) {
    ...
    /**
     * Returns the number of "No Aggro" timers created so far.
     *
     * @return the count of No Aggro timers
     */
    public long getCountNoAggro() {
        return countNoAggro;
    }

    /**
     * Returns the number of "Attack" timers created so far.
     *
     * @return the count of Attack timers
     */
    public long getCountAttack() {
        return countAttack;
    }

    /**
     * Returns the map associating each {@link Enemy} with its corresponding
     * {@link AttackListener}.
     *
     * @return a map of enemies to their attack listeners
     */
    public Map<Enemy, AttackListener> getAdjourn() {
        return Map.copyOf(adjourn);
    }

    /**
     * Returns the map of enemies to the list of timers they currently own.
     *
     * @return a map associating enemies with their active timer names
     */
    public Map<Enemy, List<String>> getEnemyTimers() {
        return Map.copyOf(enemyTimers);
    }
}
